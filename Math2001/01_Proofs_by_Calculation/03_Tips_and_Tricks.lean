...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3 = (a ^ 2 - a + 1/4) - 1/4 + 3 := by ring
    _ = (a - 1/2) ^ 2 + 11/4 := by ring
    _ = ((a - 3) + 3 - 1/2) ^ 2 + 11/4 := by ring
    _ = (2 * b + 3 - 1/2) ^ 2 + 11/4 := by rw [h1]
    _ = (2 * b  + 5/2) ^ 2 + 11/4 := by ring
    _ = 4 * b ^ 2 + 2 * 2 * 5 / 2 * b + (5/2) ^ 2 + 11/4 := by ring
    _ = 4 * b ^ 2 + 10 * b + 25/4 + 11/4 := by ring
    _ = 4 * b ^ 2 + 10 * b + 9 := by ring

-- Example 1.3.10
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = (z ^ 2 - 2 + 2) ^ 2 - (z ^ 2 - 2 + 2) * z - (z ^ 2 - 2 + 2) + 2 * z + 1 := by ring
    _ = (0 + 2) ^ 2 - (0 + 2) * z - (0 + 2) + 2 * z + 1 := by rw [h1]
    _ = 4 - 2 * z - 2 + 2 * z + 1 := by ring
    _ = 3 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 = (t ^ 2 - 4 + 4) ^ 2 + 3 * (t ^ 2 - 4 + 4) * t - 3 * (t^2 - 4 + 4) - 2 * t - 2 := by ring
    _ = (0 + 4) ^ 2 + 3 * (0 + 4) * t - 3 * (0 + 4) - 2 * t - 2 := by rw [ht]
    _ = 10 * t + 2 := by ring

example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    y = y - y * ((x + 3) - x - 3) / 2 := by ring
    _ = y - y * (5 - x - 3) / 2 := by rw [h1]
    _ = y - y * 2 / 2 + y * x / 2 := by ring
    _= y * x / 2 := by ring
    _ = - (2 * x - y * x) / 2 + x := by ring
    _ = - 0 / 2 + x := by rw [h2]
    _ = x + 3 - 3 := by ring
    _ = 5 - 3 := by rw [h1]
    _ = 2 := by ring
